...
ticker = 'EURUSD=X' #input('Input ticker symbol \n\n')
...
trailing_stop_counter = np.arange(1, TP2_ratio + 0.5, 0.5)
...
for i in range(rsi_period + 1, len(stock)):
    ...
    if (i - rsi_period) < rsi_lookback_period:
        ...
        #print(i, rsi_local, rsi_array[i - rsi_period - rsi_lookback_period : i - rsi_period])
    ...
    #print(rsi_array[i - rsi_lookback_period : i])
    #print(rsi_lookback_max, rsi_lookback_min)
    ...
    if start_time <= current_time <= end_time:
        ...
        #   counter for each individual exit scenario
        #   add long vs short trade stats
        #   total return
        #   calculate expected return and winrate
        ...
        if signal_long == 1 and position_long == 0 and overshoot_long == 0: 
            ...
            stoploss = (swing_low - entry_price_long)*SL1_ratio + entry_price_long
            risk = (entry_price_long - swing_low)*SL1_ratio
            tp1 = (entry_price_long  - swing_low)*TP1_ratio + entry_price_long
            tp2 = (entry_price_long  - swing_low)*TP2_ratio + entry_price_long
            
            upside = entry_price_long  - swing_low
        ...
        if signal_short == 1 and position_short == 0 and overshoot_short == 0: 
            ...
            stoploss = (swing_high - entry_price_short)*SL1_ratio + entry_price_short
            risk = (swing_high - entry_price_short)*SL1_ratio
            tp1 = (entry_price_short  - swing_high)*TP1_ratio + entry_price_short
            tp2 = (entry_price_short  - swing_high)*TP2_ratio + entry_price_short

            upside = entry_price_short  - swing_high

        # --Long signal--
        if rsi_local > rsi_threshold_high and rsi_lookback_min < 50 and position_long == 0 and overshoot_long == 0 and position_short == 0 and overshoot_short == 0:
            signal_long = 1
            
        # --Short signal--
        if rsi_local < rsi_threshold_low and rsi_lookback_max > 50 and position_short == 0 and overshoot_short == 0 and position_long == 0 and overshoot_long == 0:  
            signal_short = 1   
...
print(f'\nThe expected value per trade is {expected_value}*R\n')
print(f'')
...
#RSI chart
#ax2.plot(rsi_array, label = 'RSI', color = 'grey')
#ax2.axhline(rsi_threshold_high, color='green', linestyle='--', linewidth=1)
#ax2.axhline(rsi_threshold_low, color='red', linestyle='--', linewidth=1)
#ax2.set_ylabel('RSI')
#ax2.set_xlabel('Date')
...
#print(rsi_array[0:14])

#trades_df.to_csv(f"results_RSI_{ticker}.csv", index=True)

#for i in range(rsi_period, 10):
#    prev_closes = stock['Close1'].iloc[i - rsi_period : i]
 #   print(prev_closes)
...
#    for j in range(2, len(prev_closes)):
#        change_close = prev_closes[j]-prev_closes[j-1]
#        if change_close > 0:
#            rsi_gains += change_close
#        if change_close < 0:
#            rsi_losses += change_close
